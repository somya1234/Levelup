...
public class l003 {
    ...
    public static void sudoku(){
        ...
        System.out.println(sudokuBits(board, 0, locOfZeros));
    }
    ...
    //             rows[r] ^= mask; //set that number bit true in that row for further checking 
    //             cols[c] ^=mask;
    //             mat[r/3][c/3] ^= mask;
    //             board[r][c] = num;
    ...
    static int[] cols;
    static int[][] mat;
    public static int sudokuBits(int[][] board, int vidx, ArrayList<Integer> locOfZeros){
        ...
        for(int num = 1;num<=9; num++){
            ...
            if(((rows[r] & mask)==0) && ((cols[c] & mask) == 0 ) && ((mat[r/3][c/3] & mask )==0) ){
                ...
                board[r][c] = num;
                count+= sudokuBits(board, vidx+1, locOfZeros);
                board[r][c] = 0;
                rows[r] = (rows[r] & (~mask)); // mark false
                cols[c] = (cols[c] ^ mask); // or toggle.
                mat[r/3][c/3] = ( mat[r/3][c/3] ^ mask);
            }
        }
        return count;
